...
//每个table可能不同的变量字段
const tableName = globalConfig.api.invoiceList
 ...
 */
//获得找料区域、请求地点、物料号
export async function GetAreaAndLocationAndMaterialList(params) {
  return request({
    url: `${tableName}/${globalConfig.crudApi.GetAreaAndLocationAndMaterialList}`,
...
//获取配货单列表
export async function PickBillList(params) {
  return request({
    url: `${tableName}/${globalConfig.crudApi.PickBillList}`,
    // url: `http://192.168.1.116:8080/ecall/RequestMaterialBill/RequestNeedView`,
    method: 'post',
    data: params,
  })
...
//打开或关闭
export async function OpenOrClose(params) {
  return request({
    url: `${tableName}/${globalConfig.crudApi.OpenOrClose}`,
    // url: `http://192.168.1.116:8080/ecall/RequestMaterialBill/RequestNeedView`,
    method: 'post',
    data: params,
  })
