...
import { GetStationList, GetMaterialSetupRecordByCondition, addKey } from 'services/SF_PFS_Trace/TraceMaterialSetupRecord'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === `/SF_PFS_Trace/TraceMaterialSetupRecord`) {
  ...
  effects: {
    * InitQuery({
      ...
      const data = yield call(GetStationList, payload)
      console.log('traceMaterialSetupRecord-query', data)
      ...
      if (data.ReturnCode !== 0) {
        ...
        yield put({
          ...
          payload: {
            SelectInitData: data.StationListData,
            type: 'InitQuery'
          },
        })
      } else {
    ...
    * GetMaterialSetupRecordByCondition({
      payload,
    }, { call, put, select }) {
      yield put({ type: 'loadingChanger', payload: 'showLoading' })
      yield put({ type: 'tablePaginationChanger', payload: payload })
      const data = yield call(GetMaterialSetupRecordByCondition, payload)
      console.log('GetMaterialSetupRecordByCondition-query', data)
      // const pagination = yield select(state => state[TableName].pagination)
      if (data.ReturnCode !== 0) {
        return errorMessage(data.Message || '查询失败')
        yield put({ type: 'loadingChanger', payload: 'closeLoading' })
      } else if (data.ReturnCode === 0) {
        const result = yield call(addKey, data.MaterialSetupRecordData) //+1
        yield put({
          type: 'querySuccess',
          payload: {
            list: result,
