...
import { GetPartFailureRecordByPartSerialNumber, GetFailureSlipByProcessRecordId, addKey } from 'services/SF_PFS_Trace/PartFailureRecord'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === `/SF_PFS_Trace/PartFailureRecord`) {
          console.log('/SF_PFS_Trace/${PartFailureRecord}')
          // dispatch({
          //   type: 'InitQuery',
          // payload: {
          ...
          dispatch({
            ...
            payload: {
              type: 'GetPartFailureRecordByPartSerialNumber',
              WorkOrderNumber: '',
              WorkOrderPlannedQuantity: '',
              PartPartNumber: '',
              PartState: '',
          ...
          dispatch({
            type: 'InitQueryReducers',
            payload: {
              type: 'GetFailureSlipByProcessRecordId',
              TextAreaValue: ''
            },
          })
        }
  ...
  effects: {
    ...
    * GetPartFailureRecordByPartSerialNumber({
      payload,
    }, { call, put, select }) {
      // yield put({ type: 'loadingChanger', payload: 'showLoading' })
      // yield put({ type: 'tablePaginationChanger', payload: payload })
      const data = yield call(GetPartFailureRecordByPartSerialNumber, payload)
      console.log('GetPartFailureRecordByPartSerialNumber-query', data, payload)
      // const pagination = yield select(state => state[TableName].pagination)
      if (data.ReturnCode !== 0) {
        return errorMessage(data.Message || '查询失败')
        yield put({ type: 'loadingChanger', payload: 'closeLoading' })
      } else if (data.ReturnCode === 0) {
        const result = yield call(addKey, data.FailurePointData) //+1
        console.log('GetPartFailureRecordByPartSerialNumber-query', data, payload)
        yield put({
          type: 'querySuccess',
          payload: {
    ...
    * GetFailureSlipByProcessRecordId({
      payload,
    }, { call, put, select }) {
      yield put({ type: 'loadingChanger', payload: 'showLoading' })
      yield put({ type: 'tablePaginationChanger', payload: payload })
      const data = yield call(GetFailureSlipByProcessRecordId, payload)
      console.log('GetFailureSlipByProcessRecordId-query', data)
      // const pagination = yield select(state => state[TableName].pagination)
      if (data.ReturnCode !== 0) {
        return errorMessage(data.Message || '查询失败')
        yield put({ type: 'loadingChanger', payload: 'closeLoading' })
      } else if (data.ReturnCode === 0) {
        // const result = yield call(addKey, data.FailureDetailText) //+1

        yield put({
          type: 'InitQueryReducers',
          payload: {
            type: 'GetFailureSlipByProcessRecordId',
            data: data.FailureDetailText,
          },
        })
        yield put({ type: 'loadingChanger', payload: 'closeLoading' })
      } else {
  ...
  reducers: {
    ...
    InitQueryReducers(state, { payload }) {
      if (payload.type === 'GetPartFailureRecordByPartSerialNumber') {
        return {
          ...state, ...payload,
      ...
      } else if (payload.type === 'GetFailureSlipByProcessRecordId') {
        return {
          ...state, ...payload,
          TextAreaValue: payload.data
        }
      }
    },
