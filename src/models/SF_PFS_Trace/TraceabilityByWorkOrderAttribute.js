...
import { GetWorkOrderAttributeList, GetWorkOrderInformationByAttribute, addKey } from 'services/SF_PFS_Trace/TraceByAttribute'
 ...
 */
const TableName = 'traceabilityByWorkOrderAttribute'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === `/SF_PFS_Trace/TraceabilityByWorkOrderAttribute`) {
  ...
  effects: {
    * InitQuery({
      ...
      const data = yield call(GetWorkOrderAttributeList, payload)
      console.log('traceabilityByWorkOrderAttribute-query', data)
    ...
    * GetWorkOrderInformationByAttributeQuery({
      payload,
    }, { call, put, select }) {
      yield put({ type: 'loadingChanger', payload: 'showLoading' })
      yield put({ type: 'tablePaginationChanger', payload: payload })
      const data = yield call(GetWorkOrderInformationByAttribute, payload)
      console.log('GetWorkOrderInformationByAttribute-query', data)
      // const pagination = yield select(state => state[TableName].pagination)
      if (data.ReturnCode !== 0) {
        return errorMessage(data.Message || '查询失败')
        yield put({ type: 'loadingChanger', payload: 'closeLoading' })
      } else if (data.ReturnCode === 0) {
        const result = yield call(addKey, data.WorkOrderInformationData) //+1
