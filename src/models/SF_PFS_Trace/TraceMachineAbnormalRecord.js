...
import { GetStationListForCombox, GetMachineAbnormalRecord, addKey } from 'services/SF_PFS_Trace/TraceMachineAbnormalRecord'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === `/SF_PFS_Trace/TraceMachineAbnormalRecord`) {
  ...
  effects: {
    * InitQuery({
      ...
      const data = yield call(GetStationListForCombox, payload)
      console.log('GetStationListForCombox-query', data)
    ...
    * GetMachineAbnormalRecord({
      payload,
    }, { call, put, select }) {
      yield put({ type: 'loadingChanger', payload: 'showLoading' })
      yield put({ type: 'tablePaginationChanger', payload: payload })
      const data = yield call(GetMachineAbnormalRecord, payload)
      console.log('GetMachineAbnormalRecord-query', data)
      // const pagination = yield select(state => state[TableName].pagination)
      if (data.ReturnCode !== 0) {
        return errorMessage(data.Message || '查询失败')
        yield put({ type: 'loadingChanger', payload: 'closeLoading' })
      } else if (data.ReturnCode === 0) {
        const result = yield call(addKey, data.MachineAbnormalData) //+1
        yield put({
          type: 'querySuccess',
          payload: {
            list: result,
