...
import { GetPartAttributeList, GetPartInformationByAttribute, addKey } from 'services/SF_PFS_Trace/TraceByAttribute'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === `/SF_PFS_Trace/TraceabilityByPartAttribute`) {
  ...
  effects: {
    * InitQuery({
      ...
      const data = yield call(GetPartAttributeList, payload)
      console.log('traceabilityByPartAttribute-query', data)
    ...
    * GetTraceabilityByPartAttributeQuery({
      payload,
    }, { call, put, select }) {
      yield put({ type: 'loadingChanger', payload: 'showLoading' })
      yield put({ type: 'tablePaginationChanger', payload: payload })
      const data = yield call(GetPartInformationByAttribute, payload)
      console.log('GetPartInformationByAttribute-query', data)
      // const pagination = yield select(state => state[TableName].pagination)
      if (data.ReturnCode !== 0) {
        return errorMessage(data.Message || '查询失败')
