...
import { GetPartMergeRecordByPartSerialNumber, addKey } from 'services/SF_PFS_Trace/PartMergeRecord'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === `/SF_PFS_Trace/PartMergeRecord`) {
          console.log('/SF_PFS_Trace/${PartMergeRecord}')
          // dispatch({
          //   type: 'InitQuery',
          // payload: {
          ...
          dispatch({
            ...
            payload: {
              type: 'GetPartMergeRecordByPartSerialNumber',
              WorkOrderNumber: '',
              WorkOrderPlannedQuantity: '',
              PartPartNumber: '',
              PartState: '',
  ...
  effects: {
    ...
    * GetPartMergeRecordByPartSerialNumber({
      payload,
    }, { call, put, select }) {
      // yield put({ type: 'loadingChanger', payload: 'showLoading' })
      // yield put({ type: 'tablePaginationChanger', payload: payload })
      const data = yield call(GetPartMergeRecordByPartSerialNumber, payload)
      console.log('GetPartMergeRecordByPartSerialNumber-query', data, payload)
      // const pagination = yield select(state => state[TableName].pagination)
      if (data.ReturnCode !== 0) {
        return errorMessage(data.Message || '查询失败')
        yield put({ type: 'loadingChanger', payload: 'closeLoading' })
      } else if (data.ReturnCode === 0) {
        const result = yield call(addKey, data.PartMergeRecordData) //+1
        console.log('GetPartMergeRecordByPartSerialNumber-query', data, payload)
        yield put({
          type: 'querySuccess',
          payload: {
  ...
  reducers: {
    ...
    InitQueryReducers(state, { payload }) {
      if (payload.type === 'GetPartMergeRecordByPartSerialNumber') {
        return {
          ...state, ...payload,
