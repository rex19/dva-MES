...
import { GetPartAttributeRecordByPartSerialNumber, addKey } from 'services/SF_PFS_Trace/PartAttributeRecord'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === `/SF_PFS_Trace/PartAttributeRecord`) {
          console.log('/SF_PFS_Trace/${PartAttributeRecord}')
          // dispatch({
          //   type: 'InitQuery',
          // payload: {
          ...
          dispatch({
            ...
            payload: {
              type: 'GetPartAttributeRecordByPartSerialNumber',
              WorkOrderNumber: '',
              WorkOrderPlannedQuantity: '',
              PartPartNumber: '',
              PartState: '',
  ...
  effects: {
    ...
    * GetPartAttributeRecordByPartSerialNumber({
      payload,
    }, { call, put, select }) {
      // yield put({ type: 'loadingChanger', payload: 'showLoading' })
      // yield put({ type: 'tablePaginationChanger', payload: payload })
      const data = yield call(GetPartAttributeRecordByPartSerialNumber, payload)
      console.log('GetPartAttributeRecordByPartSerialNumber-query', data, payload)
      // const pagination = yield select(state => state[TableName].pagination)
      if (data.ReturnCode !== 0) {
        return errorMessage(data.Message || '查询失败')
        yield put({ type: 'loadingChanger', payload: 'closeLoading' })
      } else if (data.ReturnCode === 0) {
        const result = yield call(addKey, data.PartAttributeRecordData) //+1
        console.log('GetPartAttributeRecordByPartSerialNumber-query', data, payload)
        yield put({
          type: 'querySuccess',
          payload: {
  ...
  reducers: {
    ...
    InitQueryReducers(state, { payload }) {
      if (payload.type === 'GetPartAttributeRecordByPartSerialNumber') {
        return {
          ...state, ...payload,
