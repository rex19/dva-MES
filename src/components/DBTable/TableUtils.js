...
import configMock from '../../schema/test.config.js'
// import querySchemaMock from '../../schema/test.querySchema.js'
// import dataSchemaMock from '../../schema/test.dataSchema.js'
...
export default {
  ...
  getCacheSchema(tableName) {
    // console.log('getCacheSchema---tableMap.get(tableName)', tableMap, tableMap.get(tableName))
  ...
  getLocalSchema(tableName) {
    ...
    // console.log('TableUtils----getLocalSchema1', ignoreCache, querySchemaMock)
    let querySchema, dataSchema;

    try {
      querySchema = require(`../../schema/${tableName}.querySchema.js`);
      // querySchema = querySchemaMock;
      console.log('TableUtils----getLocalSchema2', querySchema)
      // 如果是忽略cache, 每次读取的schema都必须是全新的
      if (ignoreCache) {
        querySchema = querySchema.map(item => Object.assign({}, item));  // Object.assign是浅拷贝, 不过没啥影响
        console.log('TableUtils----getLocalSchema3===true', querySchema)
      }
    } catch (e) {
      console.log("querySchemaError", e)
      // logger.error('load query schema error: %o', e);
    }

    try {
      dataSchema = require(`../../schema/${tableName}.dataSchema.js`);
      // dataSchema = dataSchemaMock;
      console.log('TableUtils----getLocalSchema4', dataSchema)
      if (ignoreCache) {
        dataSchema = dataSchema.map(item => Object.assign({}, item));
        console.log('TableUtils----getLocalSchema5', dataSchema)
      }
    } catch (e) {
      console.log("dataSchemaError", e)
