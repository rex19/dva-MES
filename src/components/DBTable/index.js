...
class DBTable extends React.PureComponent {
  ...
  componentWillMount() {
    console.log('DBtable-componentWillMount')
    // 处理url参数
    this.processQueryParams();
    // 组件初始化时尝试获取schema
    this.tryFetchSchema(this.props, (res) => {
      console.log('tryFetchSchema', this.props, res)
      this.updateTableState(res);
   ...
   */
  componentDidMount() {
    console.log('DBtable-componentDidMount')
  ...
  componentWillReceiveProps(nextProps) {
    console.log('DBtable-componentWillReceiveProps')
    ...
    this.tryFetchSchema(nextProps, (res) => {
      console.log('tryFetchSchema1', nextProps)
      this.updateTableState(res);
      // 处理url参数
      this.state.queryObj = {};
      this.processQueryParams();
  ...
  async tryFetchSchema(props, callback) {
    console.log('tryFetchSchema2', props)
    ...
    console.log('  let tmp = TableUtils.getCacheSchema(tableName);', tmp)
    if (!tmp) {
      if (tableConfig.asyncSchema === true) {
        ...
        console.log('       if (tableConfig.asyncSchema === true) {', tmp)
      } else {
        console.log('       if (tableConfig.asyncSchema !== true) {', tmp)
    ...
    console.log(' tryFetchSchema---Return', res)
    callback(res);
  }
  ...
  updateTableState(input) {
    console.log('updateTableState+++++++++++++++++++++++++++', input)
  ...
  processQueryParams() {
    ...
    console.log('DBtable-processQueryParams', params)
    // 如果url上有参数
