...
const SchemaUtils = {
  ...
  createForm(tableName, schema) {
    const ignoreCache = TableUtils.shouldIgnoreSchemaCache(tableName);
    console.log('createForm+++++', tableName, schema.default, ignoreCache)
    // 蛋疼的this
    const that = this;
    ...
    const tmpComponent = React.createClass({
      componentWillMount() {
        ...
        if (schemaMap.has(tableName)) {
          ...
          console.log(' if (schemaMap.has(tableName)) {', this.schemaCallback)
          return;
        }
        ...
        console.log(' componentWillMount  const tmpComponent = React.createClass({---------', this.schemaCallback)
      },
      render() {
        console.log('   const tmpComponent = React.createClass({---------', this.props.form, this.schemaCallback)
        // render的时候传入getFieldDecorator, 生成最终的jsx元素
        return this.schemaCallback(this.props.form.getFieldDecorator);
      },
    });
    console.log('createForm---------', tmpComponent)
