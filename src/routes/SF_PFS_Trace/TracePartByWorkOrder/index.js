...
const TracePartByWorkOrderComponents = ({
  tracePartByWorkOrder,
  dispatch,
  location,
  form
}) => {
  //每个table可能不同的变量字段(2)
  const TableModelsData = tracePartByWorkOrder
  ...
  console.log('TableComponents-tracePartByWorkOrder ', TableModelsData)
  ...
  const handleSearch = (e) => {
    ...
    validateFields((err, values) => {
      if (!err) {
        ...
        dispatch({
          type: `${TableName}/GetPartByWorkOrder`,
          payload: Params,
        })
      }
    });
...
export default connect(({ tracePartByWorkOrder }) => ({ tracePartByWorkOrder }))(Form.create()(TracePartByWorkOrderComponents))
